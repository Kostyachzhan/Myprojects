...
class Graph:
    ...
    ###Перевіряємо унікальне з'єднання. Якщо немає просто видаємо False
    ...
    def generate_sequence_matrix(self):
        sm = []
        ...
        for i in range(self.nodes.__len__()):
            ...
            for j in range(self.nodes.__len__()):
                if i == j:
                    ...
                    row.append(self.get_distance(self.nodes[i], self.nodes[j]))
            dm.append(row)
        return dm
